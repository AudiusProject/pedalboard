...
export replica=$(nslookup $(hostname -i) | sed -n "s/^.*pedalboard-[A-z\-]*\([0-9]\)\+.*$/\1/p")
...
if [[ "$audius_redis_url" == "" ]]; then
    export audius_redis_url="redis://pedalboard-discovery-provider-redis-${replica}:6379/00"
fi

export audius_discprov_url="http://pedalboard-discovery-provider-${replica}"
...
elasticsearch_host="pedalboard-discovery-provider-elasticsearch-${replica}"
